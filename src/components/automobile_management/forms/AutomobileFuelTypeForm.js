...
const AutomobileFuelTypeForm = ({toggleModal,fetchData,perPage,type,
  automobileModelVariant,
  automobileFuelTypeUpdateData}) => {
    ...
    useEffect(() => {
        ...
        console.log(automobileFuelTypeUpdateData)
        if(type == "update") {
          const fields = ['id','name', 'description',];
          fields.forEach(field => setValue(field, automobileFuelTypeUpdateData[field]));
  ...
  const onSubmit = data => {
    ...
    if(type == "update") {
      apiClient().put(`${BACKEND_API}/v1.0/automobile-fuel-types`,data)
      .then(response => {
          console.log(response.data)
          setLoading(false)
          if(response.data) {
              SweetAlert.fire({title:"Success", text:"Automobile Fuel Type Updated Successfully!", icon:"success"});
              fetchData(perPage)
              toggleModal();
          }
      })
    ...
    if(type == "create") {
      apiClient().post(`${BACKEND_API}/v1.0/automobile-fuel-types`,data)
      .then(response => {
          console.log(response.data)
          setLoading(false)
          if(response.data) {
              SweetAlert.fire({title:"Success", text:"Automobile Fuel Type Created Successfully!", icon:"success"});
              fetchData(perPage)
              toggleModal();
          }
      })
  ...
  return (
    ...
    <Row>
        ...
        <Card>
              <CardHeader>
                <h5><span style={{textTransform:"capitalize"}}>{type}</span> Automobile Fuel Type</h5>
              </CardHeader>
              <CardBody>
                <Form className="needs-validation" noValidate="" onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <Col md="12 mb-3">
                      <Label className='text-uppercase' htmlFor="model_variant">Model Variant</Label>
                      
                      <Input className="form-control" name="automobile_model_variant_id" type="hidden" 
                      value={automobileModelVariant?.id}
                      
                      innerRef={register({ required: false })} />


                      <Input className="form-control" name="model" type="text" 
                      value={automobileModelVariant?.name}
                      readOnly
                      ...
                      {serverSideErrors?(
                        !serverSideErrors.model_variant?(
                            <div className="valid-feedback" style={{display:"block"}}>{"Looks good!"}</div>
                        ):( <div className="invalid-feedback" style={{display:"block"}}>{serverSideErrors.model_variant[0]}</div>)
                      )
                      :(null)}
...
export default AutomobileFuelTypeForm